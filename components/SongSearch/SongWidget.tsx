...
const SongWidget = ({ playTrack, volume, muted }: SongWidgetProps) => {
    ...
    const getArtistNames = (track: SongTrack) => {
        return (
            track.singers ||
            (track.artistMap ? Object.keys(track.artistMap).join(", ") : "Unknown Artist")
        )
    }
    ...
    return (
        ...
        >
            <div className="relative h-full w-full">
                <Image
                    src={playTrack.image}
                    alt={`${playTrack.album} by ${getArtistNames(playTrack)}`}
                    className={cn(
                ...
                <div className="absolute bottom-0 left-0 h-1/2 w-full bg-gradient-to-t from-black to-transparent">
                    <div className="absolute bottom-0 left-0 flex w-full items-center justify-start gap-4 p-4">
                        ...
                        <RotationText text={`${playTrack.album} • ${playTrack.year}`} />
                            <RotationText
                                text={getArtistNames(playTrack)}
